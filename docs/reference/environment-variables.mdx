...
| `TRUSTED_ORIGINS` | No | `APP_URL` | Comma-separated authentication origins. Include both `APP_URL` and `ADMIN_URL` when the admin console is exposed so native support impersonation can begin there. |
| `CORS_ALLOWED_ORIGINS` | No | `APP_URL` | Comma-separated API origins. Include both `APP_URL` and `ADMIN_URL` when the admin console calls the shared API directly. |
| `PINO_LOG_LEVEL` | No | `info` in production | Minimum structured log level. |
| `PINO_LOG_FILE` | No | Empty | Optional JSONL disk destination in addition to terminal output. Mount the parent directory to persist container logs. |
...
| Variable | Required | Default | Purpose |
| --- | --- | --- | --- |
| `OPENROUTER_API_KEY` | Only for AI features | Empty | Enables editor drafting, embeddings, and optional grounded answers through OpenRouter. |
| `AI_DRAFT_MODEL` | No | `openai/gpt-4o-mini` | OpenRouter model used for explicit editor writing actions. |
...
When `MARKETING_GTM_ID` is enabled, configure one Google tag in GTM with automatic pageviews disabled and its normal page initialization trigger. Then configure one GA4 Event tag triggered by the Custom Event `nibleaf_marketing_event`, with its GA event name read from the Data Layer Variable `event_name`. Nibleaf uses that stable envelope for the allowlisted names `page_view`, `sign_up`, `cta_clicked`, `free_tool_started`, `free_tool_completed`, and `free_tool_cta_clicked`, so six separate GTM Event tags are not required. Map only the documented safe event variables. Do not add variables that read form fields, account data, document content, or arbitrary DOM values into analytics. Keep consent checks enabled on both tags. Nibleaf loads the container only after acceptance, pushes one manual `page_view` for the accepted route and each later SPA navigation, and stops event delivery when consent is withdrawn.
...
| Variable | Required | Default | Purpose |
| --- | --- | --- | --- |
| `MCP_ENABLED` | No | `false` | Enables the authenticated project-bound `/api/mcp` endpoint. Keep it disabled until proxy and key-lifecycle checks pass. |
| `MCP_ALLOWED_HOSTS` | No | Empty | Comma-separated additional public hostnames. `API_URL` and `APP_URL` hosts are already allowed. Schemes, paths, wildcard values, and ambiguous forwarded chains are rejected. |
| `MCP_RATE_LIMIT_PER_MIN` | No | `120` | Per-key requests accepted by one API process per minute. Add an ingress/shared-edge limit for deployment-wide enforcement. |

See [secure MCP client setup and operations](/self-hosting/mcp) before enabling
the endpoint. API-key secrets are project-admin managed, displayed once, and
must not be stored in environment examples or repository configuration.
...
| Variable | Required | Default | Purpose |
| --- | --- | --- | --- |
| `INTEGRATION_CREDENTIAL_ENCRYPTION_KEY` | To create project webhook connections | None | Independent 32-byte base64 key for versioned AES-256-GCM credential envelopes and purpose-separated HMAC digests. Do not reuse the Git key. |
| `INTEGRATION_CREDENTIAL_PREVIOUS_KEYS` | During key rotation only | Empty | Comma-separated prior integration keys accepted for decrypt and digest lookup during a bounded dual-read window. |
| `INTEGRATION_CONFIRMATION_TTL_SECONDS` | No | `300` | Lifetime of a one-time, revision-bound integration delete confirmation; accepted range is 60–1800 seconds. |
| `INTEGRATION_CONFIRMATION_RETENTION_HOURS` | No | `24` | Retention for consumed confirmation tombstones before bounded cleanup; accepted range is 1–168 hours. Expired unconsumed challenges are removed on the next project challenge. |
| `INTEGRATION_IDEMPOTENCY_TTL_HOURS` | No | `24` | Retention for secret-free durable mutation replays; accepted range is 1–168 hours. |

Follow the rotation and rollback procedure in [Integrations engine](/reference/integrations-engine).
