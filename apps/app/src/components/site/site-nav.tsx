...
function NavItems({ nodes, projectId, currentPath, depth, lang }: { nodes: NavNode[]; projectId: string; currentPath: string; depth: number; lang?: string }) {
  return (
      ...
      {nodes.map((node) =>
        node.kind === 'GROUP' ? (
          <li key={node.id} className="mt-4 first:mt-0">
            ...
            {node.children.length > 0 ? <NavItems nodes={node.children} projectId={projectId} currentPath={currentPath} depth={depth} lang={lang} /> : null}
          </li>
        ) : (
          <li key={node.id}>
            <Link
            ...
            {node.children.length > 0 ? <NavItems nodes={node.children} projectId={projectId} currentPath={currentPath} depth={depth + 1} lang={lang} /> : null}
          </li>
        ),
      )}
    </ul>
