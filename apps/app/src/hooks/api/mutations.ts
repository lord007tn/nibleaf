...
import type { AiDraftResult, ApiKey, Asset, Comment, Deployment, Domain, Language, Page, Project, WorkspaceSettings } from './types';

export const useCreateProject = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (body: CreateProjectBody) => mutateData<Project>(await api.api.app.projects.$post({ json: body }), 'Could not create the site.'),
...
export const useUpdateProject = (projectId: string) => {
  ...
  return useMutation({
    mutationFn: async (body: UpdateProjectBody) =>
      mutateData<Project>(await api.api.app.projects[':id'].$patch({ param: { id: projectId }, json: body }), 'Could not update the site.'),
    onSuccess: () => {
...
export const useUpdateProjectConfig = (projectId: string) => {
  ...
  return useMutation({
    mutationFn: async ({ config, icon }: { config: ProjectConfig; icon?: string | null }) =>
      mutateData<Project>(
        await api.api.app.projects[':id'].$patch({ param: { id: projectId }, json: icon === undefined ? { config } : { config, icon } }),
...
export const useDeleteProject = () => {
  ...
  return useMutation({
    mutationFn: async (projectId: string) =>
      mutateData<{ id: string }>(await api.api.app.projects[':id'].$delete({ param: { id: projectId } }), 'Could not delete the site.'),
    onSuccess: () => qc.invalidateQueries({ queryKey: queryKeys.projects.all() }),
  });
};
...
export const useCreatePage = (projectId: string) => {
  ...
  return useMutation({
    mutationFn: async (body: CreatePageBody) =>
      mutateData<Page>(await api.api.app.projects[':projectId'].pages.$post({ param: { projectId }, json: body }), 'Could not create the page.'),
...
export const useUpdatePage = (projectId: string) => {
  ...
  return useMutation({
    mutationFn: async ({ pageId, body }: { pageId: string; body: UpdatePageBody }) =>
      mutateData<Page>(await api.api.app.projects[':projectId'].pages[':id'].$patch({ param: { projectId, id: pageId }, json: body }), 'Could not save the page.'),
...
export const useDeletePage = (projectId: string) => {
  ...
  return useMutation({
    mutationFn: async (pageId: string) =>
      mutateData<{ id: string }>(await api.api.app.projects[':projectId'].pages[':id'].$delete({ param: { projectId, id: pageId } }), 'Could not delete the page.'),
    onSuccess: () => qc.invalidateQueries({ queryKey: queryKeys.pages.allForProject(projectId) }),
  });
};
...
export const useReorderPages = (projectId: string) => {
  ...
  return useMutation({
    mutationFn: async (body: ReorderPagesBody) =>
      mutateData<unknown>(await api.api.app.projects[':projectId'].pages.reorder.$post({ param: { projectId }, json: body }), 'Could not reorder pages.'),
    onSuccess: () => qc.invalidateQueries({ queryKey: queryKeys.pages.allForProject(projectId) }),
  });
};
  ...
  return useMutation({
    mutationFn: async (message?: string) =>
      mutateData<Deployment>(
        await api.api.app.projects[':projectId'].deployments.$post({ param: { projectId }, json: message ? { message } : {} }),
        'Could not publish.',
      ),
    onSuccess: () => {
...
export const useRollback = (projectId: string) => {
  ...
  return useMutation({
    mutationFn: async (deploymentId: string) =>
      mutateData<Deployment>(
        await api.api.app.projects[':projectId'].deployments[':id'].rollback.$post({ param: { projectId, id: deploymentId } }),
        'Could not roll back.',
      ),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: queryKeys.deployments.all(projectId) });
...
export const useAddDomain = (projectId: string) => {
  ...
  return useMutation({
    mutationFn: async (body: AddDomainBody) =>
      mutateData<Domain>(await api.api.app.projects[':projectId'].domains.$post({ param: { projectId }, json: body }), 'Could not add the domain.'),
...
export const useVerifyDomain = (projectId: string) => {
  ...
  return useMutation({
    mutationFn: async (id: string) =>
      mutateData<Domain>(await api.api.app.projects[':projectId'].domains[':id'].verify.$post({ param: { projectId, id } }), 'Could not verify.'),
    onSuccess: () => qc.invalidateQueries({ queryKey: queryKeys.domains.all(projectId) }),
  });
};
...
export const useDeleteDomain = (projectId: string) => {
  ...
  return useMutation({
    mutationFn: async (id: string) =>
      mutateData<{ id: string }>(await api.api.app.projects[':projectId'].domains[':id'].$delete({ param: { projectId, id } }), 'Could not remove the domain.'),
    onSuccess: () => qc.invalidateQueries({ queryKey: queryKeys.domains.all(projectId) }),
  });
};
...
export const useCreateApiKey = (projectId: string) => {
  ...
  return useMutation({
    mutationFn: async (body: CreateApiKeyBody) =>
      mutateData<ApiKey>(await api.api.app.projects[':projectId']['api-keys'].$post({ param: { projectId }, json: body }), 'Could not create the key.'),
...
export const useRevokeApiKey = (projectId: string) => {
  ...
  return useMutation({
    mutationFn: async (id: string) =>
      mutateData<ApiKey>(await api.api.app.projects[':projectId']['api-keys'][':id'].$delete({ param: { projectId, id } }), 'Could not revoke the key.'),
    onSuccess: () => qc.invalidateQueries({ queryKey: queryKeys.apiKeys.all(projectId) }),
  });
};
  ...
  return useMutation({
    mutationFn: async (file: File): Promise<Asset> => {
      const presign = await mutateData<{ key: string; uploadUrl: string }>(
        await api.api.app.projects[':projectId'].assets.presign.$post({
          param: { projectId },
      ...
      return mutateData<Asset>(
        await api.api.app.projects[':projectId'].assets.confirm.$post({
          param: { projectId },
...
export const useInviteMember = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (body: InviteMemberBody) => mutateData<unknown>(await api.api.app.members.invite.$post({ json: body }), 'Could not send the invite.'),
...
export const useUpdateMemberRole = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, body }: { id: string; body: UpdateMemberRoleBody }) =>
      mutateData<unknown>(await api.api.app.members[':id'].role.$patch({ param: { id }, json: body }), 'Could not update the role.'),
    onSuccess: () => qc.invalidateQueries({ queryKey: queryKeys.members.all() }),
  });
};
...
export const useRemoveMember = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (id: string) => mutateData<{ id: string }>(await api.api.app.members[':id'].$delete({ param: { id } }), 'Could not remove the member.'),
    onSuccess: () => qc.invalidateQueries({ queryKey: queryKeys.members.all() }),
  });
};
...
export const useCreateComment = (projectId: string, pageId?: string) => {
  ...
  return useMutation({
    mutationFn: async (body: CreateCommentBody) =>
      mutateData<Comment>(await api.api.app.projects[':projectId'].comments.$post({ param: { projectId }, json: body }), 'Could not post the comment.'),
...
export const useResolveComment = (projectId: string) => {
  ...
  return useMutation({
    mutationFn: async ({ id, resolved }: { id: string; resolved: boolean }) =>
      mutateData<Comment>(
        await api.api.app.projects[':projectId'].comments[':id'].$patch({ param: { projectId, id }, json: { resolved } }),
...
export const useDeleteComment = (projectId: string) => {
  ...
  return useMutation({
    mutationFn: async (id: string) =>
      mutateData<{ id: string }>(await api.api.app.projects[':projectId'].comments[':id'].$delete({ param: { projectId, id } }), 'Could not delete the comment.'),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['comments', projectId] }),
  });
};
...
export const useAiDraft = (projectId: string) =>
  useMutation({
    mutationFn: async (body: AiDraftBody) =>
      mutateData<AiDraftResult>(await api.api.app.projects[':projectId'].ai.$post({ param: { projectId }, json: body }), 'Could not draft content.'),
  });
...
export const useCreateLanguage = (projectId: string) => {
  ...
  return useMutation({
    mutationFn: async (body: CreateLanguageBody) =>
      mutateData<Language>(await api.api.app.projects[':projectId'].languages.$post({ param: { projectId }, json: body }), 'Could not add the language.'),
    onSuccess: () => {
...
export const useUpdateLanguage = (projectId: string) => {
  ...
  return useMutation({
    mutationFn: async ({ id, body }: { id: string; body: UpdateLanguageBody }) =>
      mutateData<Language>(
        await api.api.app.projects[':projectId'].languages[':id'].$patch({ param: { projectId, id }, json: body }),
...
export const useDeleteLanguage = (projectId: string) => {
  ...
  return useMutation({
    mutationFn: async (id: string) =>
      mutateData<{ id: string }>(await api.api.app.projects[':projectId'].languages[':id'].$delete({ param: { projectId, id } }), 'Could not delete the language.'),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: queryKeys.languages.all(projectId) });
      qc.invalidateQueries({ queryKey: queryKeys.pages.allForProject(projectId) });
      qc.invalidateQueries({ queryKey: queryKeys.projects.detail(projectId) });
