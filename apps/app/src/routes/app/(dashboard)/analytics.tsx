...
const EMPTY = 'No traffic yet — publish a site and share it.';
...
function WorkspaceAnalyticsPage() {
  ...
  const totalDevices = Math.max(1, devices.reduce((sum, d) => sum + d.count, 0));
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-muted-foreground text-sm">Traffic and search activity across all your documentation sites.</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard label="Page views" value={data?.totalViews ?? 0} icon={<BarChart3 className="size-4" />} loading={isPending} />
        <StatCard label="Unique visitors" value={data?.uniqueVisitors ?? 0} icon={<Users className="size-4" />} loading={isPending} />
        <StatCard label="Searches" value={data?.searches.total ?? 0} icon={<Search className="size-4" />} loading={isPending} />
        <StatCard label="Projects with traffic" value={projectsWithTraffic} icon={<Activity className="size-4" />} loading={isPending} />
      </div>

      {/* Pageviews chart */}
      <div className="rounded-xl border border-border bg-card p-5">
        <div className="mb-4 font-medium text-sm">Pageviews · all projects</div>
        {isPending ? (
          ...
          <ResponsiveContainer height={260} width="100%">
            <BarChart data={data?.timeseries ?? []} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
              ...
              <Tooltip cursor={{ fill: 'var(--muted)' }} contentStyle={{ background: 'var(--popover)', border: '1px solid var(--border)', borderRadius: 10, fontSize: 12 }} />
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-border bg-card p-5">
          <div className="mb-3 font-medium text-sm">By project</div>
          {isPending ? (
      ...
      {/* Top pages */}
      <ListCard
        title="Top pages across all docs"
        loading={isPending}
        empty={EMPTY}
        rows={6}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-border bg-card p-5">
          <div className="mb-3 font-medium text-sm">Devices</div>
          {isPending ? (
            <div className="space-y-2.5">
              {[0, 1, 2].map((i) => (
                <Skeleton key={i} className="h-9 w-full" />
            ...
            <div className="-mx-2">
              {devices.map((d) => (
                <BarRow key={d.device} label={<span className="capitalize">{d.device}</span>} fraction={d.count / totalDevices} value={d.count.toLocaleString()} />
              ))}
            </div>
          )}
        </div>
