...
faqs:
  - question: 'Why can Arabic documentation search miss a word that is on the page?'
    answer: 'An English tokenizer can drop Arabic text entirely. Nibleaf selects Orama’s Arabic tokenizer and normalizes common alef forms, diacritics, tatweel, and alef maqsura. It does not enable stemming, so prefixed or inflected forms still need explicit tests.'
...
The implementation deliberately does not fold ta marbuta `ة` into ha `ه`. They are different letters, and merging them can change the words returned. It also keeps stemming off. Do not assume `مستخدم` and `للمستخدمين` will match. Prefixes and suffixes need their own tests and, if the project requires it, a carefully evaluated stemming strategy.

This distinction keeps the product claim honest: Arabic text is indexed with an Arabic tokenizer, and common orthographic variants receive limited normalization. That is not the same as full morphological search.
...
Nibleaf currently provides per-language trees, RTL layout, inline-code isolation, Arabic tokenization with limited normalization, `hreflang`, and an English and Arabic interface. It does not currently enable Arabic stemming, and it cannot repair inconsistent terminology or weak translations.

For an Arabic version of this checklist, read [كيف تنشر وثائق منتج بالعربية](/blog/arabic-technical-documentation-rtl-checklist). You can also [start on the hosted beta](/sign-up). The public self-hosting instructions should be treated as ready only when the repository and container image are anonymously accessible.
