...
This guide gives you a production checklist for a Compose-based docs stack. It uses Nibleaf's architecture as the concrete example, but it does not pretend the current public installation path works.

<Callout title="Current Nibleaf distribution status">
The Nibleaf repository and GHCR container reject anonymous access as of August 15, 2026. The deployment design exists, but a new public user cannot fetch the required artifacts. Treat this as an architecture guide, not an install command. The shorter [self-hosting status page](/self-hosting) will be updated when both resources are publicly retrievable.
</Callout>
...
For Nibleaf specifically, do not schedule a migration until the [public distribution checks](/self-hosting) pass. The right next step is to verify artifact access first, then perform this checklist on a disposable server before moving real documentation.
