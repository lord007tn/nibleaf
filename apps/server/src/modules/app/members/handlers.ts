...
const app = new Hono<HonoEnv>()
  ...
  .post('/invite', ...membersRoutes.invite, validator('json', inviteMemberBody), async (ctx) => {
    ...
    return ctx.json({ data: await inviteMember(organizationId, user.id, ctx.req.valid('json')) }, 201);
  })
  .patch('/:id/role', ...membersRoutes.updateRole, validator('json', updateMemberRoleBody), async (ctx) => {
    const organizationId = getContextOrganizationIdOrThrow();
    return ctx.json({ data: await updateMemberRole(organizationId, ctx.req.param('id'), ctx.req.valid('json')) }, 200);
  })
  .delete('/:id', ...membersRoutes.remove, async (ctx) => {
    const organizationId = getContextOrganizationIdOrThrow();
    return ctx.json({ data: await removeMember(organizationId, ctx.req.param('id')) }, 200);
  })
