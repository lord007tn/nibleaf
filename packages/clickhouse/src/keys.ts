...
export const keys = () =>
  createEnv({
    server: {
      ...
      CLICKHOUSE_REQUEST_TIMEOUT_MS: z.coerce.number().int().min(250).max(60_000).default(5_000),
      ...
      CLICKHOUSE_FLUSH_INTERVAL_MS: z.coerce.number().int().min(25).max(30_000).default(1_000),
      CLICKHOUSE_RAW_RETENTION_DAYS: z.coerce.number().int().min(7).max(730).default(180),
      CLICKHOUSE_ROLLUP_RETENTION_DAYS: z.coerce.number().int().min(30).max(3_650).default(730),
