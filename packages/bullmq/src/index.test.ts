...
const { queueMocks } = vi.hoisted(() => ({
  queueMocks: {
    analytics: { add: vi.fn(), getJob: vi.fn(), getJobSchedulers: vi.fn(async () => []), upsertJobScheduler: vi.fn() },
    export: { add: vi.fn(), getJob: vi.fn(), getJobSchedulers: vi.fn(async () => []), upsertJobScheduler: vi.fn() },
  },
}));
...
describe('BullMQ 6 job schedulers', () => {
  ...
  it('upserts analytics schedulers with stable ids and UTC cron definitions', async () => {
    ...
    expect(queueMocks.analytics.upsertJobScheduler).toHaveBeenNthCalledWith(
      ...
      { pattern: '10 0 * * *', tz: 'UTC' },
      { name: 'rollup-analytics', data: {}, opts: QUEUE_CONFIGS.analytics.defaultJobOptions },
    );
    expect(queueMocks.analytics.upsertJobScheduler).toHaveBeenNthCalledWith(
      2,
      'reconcile-usage-periods',
      { pattern: '*/5 * * * *', tz: 'UTC' },
      { name: 'reconcile-usage', data: {}, opts: QUEUE_CONFIGS.analytics.defaultJobOptions },
    );
