...
Visible runtime chrome is loaded from `messages/en.json` and `messages/ar.json` through generated Paraglide message functions. Language and version controls scope every navigation surface and the local snapshot search to matching pages. Scope changes preserve the current `translationKey` where a translation exists, fall back only inside the selected scope, and keep controls/search available for an empty scope. Selecting Arabic switches messages, logical control order, the template root direction, and the document `lang`/`dir`. Snapshot search has no provider, filesystem, or network coupling. Search, external-link, book, and terminal imagery uses Lucide components rather than Unicode icon glyphs.
